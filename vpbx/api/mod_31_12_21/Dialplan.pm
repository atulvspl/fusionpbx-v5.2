...
';
	...
	if (!$response{stat} ne 'fail') {
		...
		if (!1) {
			...
			$response{stat}		= "ok";
			# Date :22-Mar-2021 Added by Atul for return domain_uuid 
			$sql="SELECT 1,dialplan_uuid,app_uuid from v_dialplans where domain_uuid='$domain{uuid}' and dialplan_name='$post_add{dialplan_name}' and dialplan_context='$post_add{dialplan_context}'";
 			%data = &database_select_as_hash($sql,"dialplan_uuid");
			$dialplan_uuid=$data{1}{dialplan_uuid};
			$response{data}{dialplan_uuid}=$dialplan_uuid;
