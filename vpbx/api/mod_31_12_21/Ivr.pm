...
';
    ...
    %ivr_ext = &database_select_as_hash(
                "select
                    1,ivr_menu_extension
                from
                    v_ivr_menus
                where
                    (ivr_menu_name='$post_add{ivr_menu_name}' or ivr_menu_extension='$post_add{ivr_menu_extension}') and
	...
	if ($ivr_ext{1}){
		...
		$response{stat}		= "fail";
		$response{message}	= "IVR Already Exists. Please add IVR Extension and IVR name must be unique";
		...
		if ($response{stat} ne 'fail') {
			...
			# Date :-15-Mar-2021 Added by Atul for insert the ivr details 
				$post_add{domain_uuid}=> $domain{uuid};
			#END 
			
			$result = &post_data ('domain_uuid' => $domain{uuid},
						'urlpath'     => '/app/ivr_menus/ivr_menu_edit.php',
						'reload'	  => 0,
						'data'        => [%post_add]
						);
...
';
	...
	#Code Added By Hemant Chaudhari
    %ivr_ext_edt = &database_select_as_hash(
                "select
                    1,ivr_menu_extension
                from
                    v_ivr_menus
                where
                    (ivr_menu_name='$post_add{ivr_menu_name}' or ivr_menu_extension='$post_add{ivr_menu_extension}') and ivr_menu_uuid != '$post_add{ivr_menu_uuid}' and
					domain_uuid='$domain{uuid}'",
                'uuid'
        );
	...
	if ($ivr_ext_edt{1}){
		...
		if ($response{stat} ne 'fail') {		
			...
			%result = &post_data ('domain_uuid' => $domain{uuid},
						'urlpath'     => "/app/ivr_menus/ivr_menu_edit.php?id=$post_add{ivr_menu_uuid}",
						'reload'	  => 1,
						'data'        => [%post_add]
						);
...
sub deleteivroption () {
	...
	if (!$hash{1}{uuid}) {
		...
		&post_data ( 'domain_uuid' => $domain{uuid},
						'urlpath'     => "/app/ivr_menus/ivr_menu_option_delete.php?id=$ivr_menu_option_uuid&ivr_menu_uuid=$ivr_menu_uuid",
						'reload'      => 1,                    
						'data'        => []
						);
		$response{stat}    = "ok";
...
sub deleteivr() {
	...
	if (!$hash{1}{uuid}) {
		...
		&post_data ( 'domain_uuid' => $domain{uuid},
					'urlpath'     => "/app/ivr_menus/ivr_menu_delete.php?id=$ivr_menu_uuid",
					'reload'      => 1,                    
					'data'        => []
				);
		$response{stat}    = "ok";
