...
sub get_pre_recorded_vm_list()
{
	...
	#%hash=  &database_select_as_hash("select recording_uuid,recordings_filename from v_pre_recorded_voicemail pre_vm,v_extensions v_ext where v_ext.extension_uuid=pre_vm.extension_uuid and v_ext.extension='$ext' and accountcode='$domain_name' 

	%hash=  &database_select_as_hash("select recording_uuid,recordings_filename from v_pre_recorded_voicemail pre_vm,v_extensions v_ext where v_ext.extension_uuid=pre_vm.extension_uuid and v_ext.extension='$ext' and v_ext.accountcode='$domain_name' and v_ext.domain_uuid='$domain{uuid}' 
...
sub set_pre_recorded_vm_to_extension()
{
	...
	$sth = $dbh->prepare("UPDATE v_extensions set pre_recorded_voicemail_uuid='$recording_uuid' where v_extensions.accountcode='$domain_name' and v_extensions.extension='$ext'");
	$sth->execute();
	$response{stat}        = 'ok';
...
sub rename_pre_recoded_vm_filename()
{
	...
	if(($new_file_name =~ /\.mp3$/i)||($new_file_name =~/\.wav$/i)) {
		...
		$dbh = DBI->connect($app{database_dsn}, $app{database_user}, $app{database_password});
		use DBD::Pg qw(:pg_types);
		$sth = $dbh->prepare("UPDATE v_pre_recorded_voicemail set recordings_filename='$new_file_name' ,recording_name='$recording_name' where v_pre_recorded_voicemail.recording_uuid='$recording_uuid'");
		$sth->execute();
...
sub get_contactbook()
{
	...
	%hash=  &database_select_as_hash(
		"select extension, directory_full_name from v_domains,v_extensions where v_domains.domain_uuid = v_extensions.domain_uuid and v_domains.domain_name='$domain_name' ",'extension,directory_full_name');
