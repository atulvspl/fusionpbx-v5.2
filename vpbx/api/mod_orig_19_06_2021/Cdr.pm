...
#Developed by :-Atul Akabari 
sub check_remotedb_connection()
{

	use DBI; 
	my $remote_dbname = 'fusionpbx';  
	my $remote_host = '44.237.233.39';  
	my $remote_port = 5432;  
	my $remote_username = 'fusionpbx';  
	my $remote_password = 'ceMPSOGqqKJzrzqfIfQTGC6e0k';  
	...
	$query = $dbh->prepare("SELECT * FROM v_domains");
	$result = $query->execute();

	$response{stat} = 'ok';
	$response{stat1} = $result;
